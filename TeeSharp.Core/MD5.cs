//https://raw.githubusercontent.com/erdomke/Innovator.Client/master/src/Innovator.Client/Security/Md5.cs
using System;
using System.IO;
using System.Text;
...
namespace TeeSharp.Core
{
    ...
    public class MD5
    {
        ...
        {
            var digest = new MD5().CalculateMD5Value(value);
            var array = new byte[16];
            Buffer.BlockCopy(new [] {digest.A, digest.B, digest.C, digest.D}, 0, array, 0, 16);
            return array;
        }

        public static string ComputeHash(string value)
        {
            return new MD5().CalculateMD5Value(Encoding.UTF8.GetBytes(value)).ToString().ToLowerInvariant();
        }

        public static string ComputeHash(MemoryStream value)
        {
            return new MD5().CalculateMD5Value(value.ToArray()).ToString().ToLowerInvariant();
        }

        /// <summary>
        /// calculate md5 signature of the string in Input
        /// </summary>
        /// <returns> Digest: the finger print of msg</returns>
        public Digest CalculateMD5Value(byte[] value)
        {
            var dg = new Digest(); //  the value to be returned
            // create a buffer with bits padded and length is also padded
            var bMsg = CreatePaddedBuffer(value);
         ...
         * where each Each letter represents the aux function used
         *********************************************************/
        ...
        protected byte[] CreatePaddedBuffer(byte[] value)
        {
            var temp = (448 - ((value.Length * 8) % 512)); //temporary
            var pad = (uint) ((temp + 512) % 512);
            if (pad == 0) // pad is in bits
                pad = 512; //at least 1 or max 512 can be added

            var sizeMsgBuff = (uint) ((value.Length) + (pad / 8) + 8);
            var sizeMsg = (ulong) value.Length * 8;
    ...
    /// <summary>
    /// helper class providing supporting function
    /// </summary>
