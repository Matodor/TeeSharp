...
public class NetworkServerOld : BaseNetworkServer
{
    ...
    public override bool Receive(out NetworkMessage netMsg, ref SecurityToken responseToken)
    {
        ...
        if (ChunkFactoryOld.NetworkPacket.Flags.HasFlag(PacketFlags.ConnectionLess))
        {
            ...
            netMsg = new NetworkMessage
            {
                ...
                Data = new byte[ChunkFactoryOld.NetworkPacket.DataSize],
                ExtraData = null,
            };

            ChunkFactoryOld.NetworkPacket.Data
                .AsSpan()
                .Slice(0, ChunkFactoryOld.NetworkPacket.DataSize)
                .CopyTo(netMsg.Data);

            if (ChunkFactoryOld.NetworkPacket.Flags.HasFlag(PacketFlags.Extended))
            {
                netMsg.Flags |= MessageFlags.Extended;
                netMsg.ExtraData = new byte[NetworkConstants.PacketExtraDataSize];

                ChunkFactoryOld.NetworkPacket.ExtraData
                    .AsSpan()
        ...
        if (connectionId == -1)
        {
            ...
            if (IsConnStateMsgWithToken(ChunkFactoryOld.NetworkPacket))
            {
                ProcessConnStateMsgWithToken(endPoint, ChunkFactoryOld.NetworkPacket);
                return false;
            }
